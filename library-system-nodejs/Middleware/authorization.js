...
module.exports.checkAdminAndBasicAdmin=(request,response,next)=>{
  ...
  module.exports.checkAll=(request,response,next)=>{
    if(request.role=="admin" || request.role=="basicAdmin" || request.role=="empolyee" || request.role=="Root"|| request.role=="member")
    {
      //go to next layer  (controller)
        next();
    }
